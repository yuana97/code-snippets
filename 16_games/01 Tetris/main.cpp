...
int main()
{
  ...
  Texture t1, t2, t3;
	t1.loadFromFile("/Users/alleyuan/games/01 Tetris/images/tiles.png");
	t2.loadFromFile("/Users/alleyuan/games/01 Tetris/images/background.png");
	t3.loadFromFile("/Users/alleyuan/games/01 Tetris/images/frame.png");
  ...
  while (window.isOpen()) // game loop
  {
    ...
    // LINES
    int k = M-1; // row to draw into
    for (int i = M-1; i > 0; i--) // skip full rows
    {
      int count = 0;
      for (int j = 0; j < N; j++)
      {
      ...
      if (count < N) k--; // not filled -> draw it
    }
    ...
    for (int i = 0; i < M; i++) // paint all blocks
      for (int j = 0; j < N; j++)
      {
        ...
        s.move(28,31); // offset into frame
        window.draw(s);
      }
